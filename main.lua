...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			exports['okokNotify']:Alert("Polizei", "Kein Outfit", 5000, 'warning')
		end
	end)
end
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('hemd_wear'), uniform = 'hemd_wear'},
		{label = _U ('jacket_wear'), uniform = 'jacket_wear'},
		{label = _U('highway_wear'), uniform = 'highway_wear'},
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elements = {
			...
			{label = 'Arbeitswerkzeuge', value = 'object_spawner'},
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				--{label = 'Ins Gefängnis transferieren', value = 'pjail_menu'},
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'search' then
						TriggerEvent("esx_inventoryhud:openPlayerInventory", GetPlayerServerId(closestPlayer), GetPlayerName(closestPlayer)) OpenBodySearchMenu(closestPlayer)
					...
					exports['okokNotify']:Alert("Polizei", "Kein Spieler in der Nähe", 5000, 'warning')
				end
		...
		elseif data.current.value == 'vehicle_interaction' then
			...
			if DoesEntityExist(vehicle) then
				...
				table.insert(elements, {label = 'Flugzeug Impound', value = 'fimpound'})
			end
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'hijack_vehicle' then
						if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
							...
							exports['okokNotify']:Alert("Polizei", "Fahrzeug aufgebrochen", 5000, 'success')
						end
					elseif action == 'impound' then
						...
						exports['okokNotify']:Alert("Polizei", "Fahrzeug beschlagnahmen", 5000, 'success')
						TaskStartScenarioInPlace(playerPed, 'CODE_HUMAN_MEDIC_TEND_TO_DEAD', 0, true)

						currentTask.busy = true
						currentTask.task = ESX.SetTimeout(10000, function()
							ClearPedTasks(playerPed)
							TriggerServerEvent('ps_impound:changeState', GetVehicleNumberPlateText(vehicle), 0)
						...
						Citizen.CreateThread(function()
							while currentTask.busy do
								...
								if not DoesEntityExist(vehicle) and currentTask.busy then
									...
									exports['okokNotify']:Alert("Polizei", "Beschlagnahmung abgebrochen", 5000, 'success')
					...
					elseif action == 'fimpound' then
						...
						currentTask.task = ESX.SetTimeout(10000, function()
							ClearPedTasks(playerPed)
							TriggerServerEvent('nv_impound_plane:changeState', GetVehicleNumberPlateText(vehicle), 1)
							ImpoundVehicle(vehicle)
							Citizen.Wait(100) -- sleep the entire script to let stuff sink back to reality
						end)
						...
						Citizen.CreateThread(function()
							while currentTask.busy do
								...
								if not DoesEntityExist(vehicle) and currentTask.busy then
									--ESX.ShowNotification(_U('impound_canceled_moved'))
									exports['okokNotify']:Alert("Polizei", "test", 5000, 'info')
									ESX.ClearTimeout(currentTask.task)
									ClearPedTasks(playerPed)
									currentTask.busy = false
									break
					...
					exports['okokNotify']:Alert("Polizei", "Kein Fahrzeug in der Nähe", 5000, 'info')
				end
...
-- Eigene Rechnungen austellen by [Rok#0001]
...
function OpenRoksFineMenu(player)
	...
	function(data, menu)
	  ...
	  if reason == nil then
			...
			exports['okokNotify']:Alert("Polizei", "Du musst einen Grund angeben", 5000, 'info')
	  else
		menu.close()
		...
		ESX.UI.Menu.Open(
			...
			function(data2, menu2)
				...
				if price == nil then
					...
					exports['okokNotify']:Alert("Polizei", "Du musst einen Betrag angeben", 5000, 'info')
				else
					menu2.close()
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_policejob:getVehicleFromPlate', function(owner, found)
				if found then
					...
					exports['okokNotify']:Alert("Polizei", "Das Kennzeichen ist keiner Person zugewiesen", 5000, 'info')
				end
			end, data.value)
			menu.close()
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					exports['okokNotify']:Alert("Polizei", "Ungültige Menge", 5000, 'info')
				else
					menu2.close()
					menu.close()
...
Citizen.CreateThread(function()
	while true do
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'menu_armory' then
					...
					elseif playerInService then
						...
						exports['okokNotify']:Alert("Polizei", "Du musst zuerst den Dienst antreten", 5000, 'info')
					end
		...
		if IsControlJustReleased(0, 38) and currentTask.busy then
			...
			exports['okokNotify']:Alert("Polizei", "Beschlagnahmung abgebrochen", 5000, 'info')
			ESX.ClearTimeout(currentTask.task)
...
--Kleidung Jail
RegisterNetEvent('esx_policejob:kleidungjail')
AddEventHandler('esx_policejob:kleidungjail', function(source)
...
function ImpoundVehicle(vehicle)
	...
	exports['okokNotify']:Alert("Polizei", "Fahrzeug beschlagnahmt", 5000, 'success')
	currentTask.busy = false
end
